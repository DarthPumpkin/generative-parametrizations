...
class LSTM_Dataset(Dataset):
    def __init__(self, episodes: Sequence[Tuple[np.ndarray]], window_size, observations: Sequence[np.ndarray]=None):
        ...
        self.out_state = observations  # n_episodes x episode_horizon x state_size
        if not self.in_state.shape[:2] == self.actions.shape[:2] == self.out_state.shape[:2]:
            raise ValueError("Misaligned shapes")
        self.episode_horizon = self.in_state.shape[0]
        self.window_size = window_size
        windows_per_espisode = self.episode_horizon - window_size + 1
        self.total_windows = self.episode_horizon * windows_per_espisode
    ...
    def __getitem__(self, item):
        if item > self.total_windows:
            raise IndexError()
        episode_ix = item // self.episode_horizon
        offset = item % self.episode_horizon
